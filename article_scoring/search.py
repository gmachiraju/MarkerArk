...
def getAuthors(_filename):
    ...
    generator_authorsCited_reg_normal = re.finditer("(?<=\s)[A-Z][a-zA-Z]{1,}\s[A-Z]{1,4}(?=[.,])|(?<=^)[A-Z][a-zA-Z]{1,}\s[A-Z]{1,4}(?=[.,])",shorter_text)
    generator_authorsCited_reg_dash = re.finditer("(?<=\s)[A-Z][a-zA-Z]{1,}-[A-Z][a-zA-Z]{1,}\s[A-Z]{1,3}(?=[.,])|(?<=^)[A-Z][a-zA-Z]{1,}-[A-Z][a-zA-Z]{1,}\s[A-Z]{1,3}(?=[.,])",shorter_text)
        ...
        for author in articleCitations:
            ...
            for authorObj in authorList:
                ...
                if author == authorObj[1]:
                    authorList[objIndex][0] = authorList[objIndex][0] + 1 #increase author score by one
                    noMatch = False
            if noMatch:
                authorList.append([1,author])
...
def getFinalAuthorScore(authorList):
    ...
    for author in authorList:
        newScore = (len(authorList) - authorList.index(author))/len(authorList) * 100
