...
import LevelsRelation_BiomarkerLevels_Extraction, LevelsRelation_BiomarkerMeasurementType_Extraction, LevelsRelation_BiomarkerTestSet_Extraction, LevelsRelation_BiomarkerUnits_Extraction
...
def generateRelations(_filename):
    ...
    possiblePairs = LevelsRelation_BiomarkerLevels_Extraction.generateRelations(sentences)

    return possiblePairs


def generateAllTupleRelations(_filename):
    filename = _filename

    sentences = doc_parser.parseDoc(filename)

    possiblePairs_BM_L = LevelsRelation_BiomarkerLevels_Extraction.generateRelations(sentences)
    possiblePairs_BM_TS =  LevelsRelation_BiomarkerTestSet_Extraction.generateRelations(sentences)
    possiblePairs_BM_MT = LevelsRelation_BiomarkerMeasurementType_Extraction.generateRelations(sentences)
    possiblePairs_BM_U = LevelsRelation_BiomarkerUnits_Extraction.generateRelations(sentences)
                    ...
                    if pair_BM_L.mention1(attribute='sent_id') == pair_BM_MT.mention1(
                        ...
                        for pair_BM_U in possiblePairs_BM_U:
                            if pair_BM_L.mention1(attribute='sent_id') == pair_BM_U.mention1(
                                ...
                                multiRelation = [pair_BM_L, pair_BM_TS, pair_BM_MT, pair_BM_U]
