...
############################ BINARY SEARCH ############################
####################################################################
#####NOTE THAT A WORD BEING GREATER THAN ANOTHER MEANS THAT IT IS ALPHABETICALLY AFTER#####
...
def get_next_available_biomarker_name_pk_key(db):
    cursor = db.cursor()
    sql = "SELECT * FROM MARKERVILLE.Biomolecule_Names \
          ORDER BY pk_Biomolecule_Names DESC"
    cursor.execute(sql)
...
def add_biomarker(db, name):
    ...
    if not (biomarker_existence[0]):
        cursor.execute("INSERT INTO MARKERVILLE.Biomolecule_Names(pk_Biomolecule_Names, fk_Biomolecules, Name) VALUES (%d, %d, '%s')" % (nextPk, nextFk, name))
        return nextPk
    else:
        return biomarker_existence[1]
def get_next_available_biomarker_pk(db):
    cursor = db.cursor()
    sql = "SELECT * FROM MARKERVILLE.Biomolecules \
          ORDER BY pk_Biomolecules DESC"
    cursor.execute(sql)
    results = cursor.fetchall()
    nextavailable = results[0][0]
    return nextavailable + 1
def add_biomolecule_v2(db, name, medium, type):
    ...
    cursor = db.cursor()
    sql = "INSERT INTO MARKERVILLE.Biomolecules(pk_Biomolecules, fk_Biomolecule_Medium, fk_Biomolecule_Type) VALUES (%d, %d, %d)" % (pk_Biomolecules, fk_Biomolecule_Medium, fk_Biomolecule_type)
    cursor.execute(sql)
    sql = "INSERT INTO MARKERVILLE.Biomolecules_Names(pk_Biomolecule_Names, fk_Biomolecules, Name) VALUES (%d, %d, '%s')" % (get_next_available_biomarker_name_pk_key, pk_Biomolecules, name)
    cursor.execute(sql)
...
def get_next_available_medium_pk_key(db):
    cursor = db.cursor()
    sql = "SELECT * FROM MARKERVILLE.Biomolecule_Medium \
          ORDER BY pk_Biomolecule_Medium DESC"
    cursor.execute(sql)
    results = cursor.fetchall()
    nextavailable = results[0][0]
    return nextavailable + 1
...
def add_medium(db, name):
    ...
    if not (medium_existence[0]):
        cursor.execute("INSERT INTO MARKERVILLE.Biomolecule_Names(pk_Biomolecule_Medium, Medium) VALUES (%d, '%s')" % (nextPk, name))
        return nextPk
    else:
...
def get_next_available_type_pk(db):
    cursor = db.cursor()
    sql = "SELECT * FROM MARKERVILLE.Biomolecule_Type \
          ORDER BY pk_Biomolecule_Type DESC"
    cursor.execute(sql)
    results = cursor.fetchall()
    nextavailable = results[0][0]
    return nextavailable + 1
...
def get_next_available_source_pk(db):
    cursor = db.cursor()
    sql = "SELECT * FROM MARKERVILLE.Sources \
              ORDER BY pk_Sources DESC"
    cursor.execute(sql)
    results = cursor.fetchall()
    # print results
    nextavailable = results[0][0]
...
def get_next_available_levels_pk(db):
    cursor = db.cursor()
    sql = "SELECT * FROM MARKERVILLE.Levels \
                  ORDER BY levels_pk DESC"
    cursor.execute(sql)
    results = cursor.fetchall()
    # print results
    nextavailable = results[0][0]
...
def add_levels(db, level, test_set, measurement_type, unit):
    ...
    sql = "INSERT INTO MARKERVILLE.Levels(levels_pk, levels, test__set, measurement_type, units) VALUES(%d, %d, '%s', '%s', '%s')" % (next_available_pk, level, test_set, measurement_type, unit)
    cursor.execute(sql)
    ...
    for tuple in alt_names_db:
        if (len(tuple) >= 2):
            ...
            while(count < len(tuple) - 1):
                sql1 += "'" + tuple[count] + "' OR Biomolecule_Names='" + tuple[count + 1] + "'"
                sql2 += "'" + tuple[count] + "' OR Biomolecule_Names='" + tuple[count + 1] + "'"
                count += 2
            cursor.execute(sql1 + "ORDER BY fk_Biomolecules ASC")
            results = cursor.fetchall()
            best = results[0][1]
            cursor.execute("UPDATE MARKERVILLE.Biomolecule_Names SET fk_Biomolecules=" + best + " " + sql2)
...
def get_next_available_relations_pk(db):
    cursor = db.cursor()
    sql = "SELECT * FROM MARKERVILLE.Biomolecules_Sources_Association \
                     ORDER BY pk_Biomolecules_Sources_Association DESC"
    cursor.execute(sql)
    results = cursor.fetchall()
    # print results
    nextavailable = results[0][0]
